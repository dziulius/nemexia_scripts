...
unsafeWindow.prepareFleet = function(cordinates, callback) {
    $.post("ajax_fleets.php", "type=shipsCheck&ship[2][11]=1&mission=8", function(response) {
...
unsafeWindow.searchAsteroids = function() {
    ...
    scanSystem(galaxy, solar_from, solar_to, ".planet img[src$='asteroid.png']", 2000, function(text, cordinates) {
        ...
        prepareFleet(cordinates, function(flightTime) {
            ...
            var destination = "type=SendFleet&ship[2][11]=1&mission=8&speed=10&metal=0&crystal=0&gas=0&scrap=0&" + "c1=" + cordinates[0] + "&c2=" + cordinates[1] + "&c3=" + cordinates[2] + "&battle_rounds=12&speed_motivation=0&scrap_motivation=0&flight_hours=0&flight_minutes=5";
